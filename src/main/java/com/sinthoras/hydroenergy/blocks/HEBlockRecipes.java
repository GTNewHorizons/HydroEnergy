    ...
    public static void registerRecipes() {
        ...
        for (Materials material : Materials.values()) {
            if (material.mStandardMoltenFluid != null
                    && material.contains(SubTag.SOLDERING_MATERIAL)
                    && !(GregTech_API.mUseOnlyGoodSolderingMaterials
                            && !material.contains(SubTag.SOLDERING_MATERIAL_GOOD))) {
                int multiplier = material.contains(SubTag.SOLDERING_MATERIAL_GOOD)
                        ? 1
                        ...
                        GT_Values.RA.addAssemblerRecipe(
                                new ItemStack[] {
                                    ...
                                    GT_OreDictUnificator.get(
                                            OrePrefixes.cableGt01, cableMaterialsPerVoltage[tierId], 2L),
                                    GT_Utility.getIntegratedCircuit(1)
                                },
                                material.getMolten(144L * multiplier / 2L),
