...
public class GSPlanetFogHandlerTransformer implements IClassTransformer {
    ...
    public static byte[] transform(byte[] basicClass, boolean isObfuscated) {
        ...
        if(isApiUsed) {
            HEUtil.info("Galaxy Space(" + fullClassName + ") is using HydroEnergy API. No injection necessary.");
            return basicClass;
        }
        HEUtil.info("Galaxy Space(" + fullClassName + ") does not use HydroEnergy API. Fallback to injection.");
        ...
        HEUtil.info("Fixed mod-interop with Galaxy Space.");
